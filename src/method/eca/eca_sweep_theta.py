    ...
    def run_Tx(self):
        ...
        for _deg in range(1, 6, 1):
            ...
            for _idx_x in range(1, 11, 2):
                ...
                print(Tx, deg, f"{rate*100:.6f}")
                result_txty.append([Tx, deg, rate*100, max(rate*100-90,0)])
        ...
        ddf = pd.DataFrame(result_txty, columns=["x", "deg", "res1", "res2"])
        Z = ddf.pivot(index="deg", columns="x", values="res2")
    ...
    def run_Ty(self):
        ...
        for _deg in range(1, 6, 1):
            ...
            for _idx_y in range(1, 11, 2):
                ...
                rate = match_rate_get_df(ode_file_path, df)
                print(Ty, deg, f"{rate*100:.6f}")
                result_txty.append([Ty, deg, rate*100, max(rate*100-90,0)])
        ...
        ddf = pd.DataFrame(result_txty, columns=["y", "deg", "res1", "res2"])
        Z = ddf.pivot(index="deg", columns="y", values="res2")
