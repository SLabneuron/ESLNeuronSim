...
def calc_time_evolution_eca(init_X, init_Y, init_P, init_Q, init_phX, init_phY,
    ...
    store_step = (total_step - index_start) + 1 if total_step > index_start else 0
    ...
    def dde_X():
        ...
        Fin = np.where(np.abs(Fin) <= 0.001, 0, gamma_X/Fin*(Tc/Tx))
    ...
    def dde_Y():
        ...
        Gin = np.where(np.abs(Gin) <= 0.001, 0, gamma_Y/Gin*(Tc/Ty))
    ...
    def time_evolution(T, phx_previous, phy_previous):
    ...
    for i in range(total_step):
        ...
        # time evolution
        T, phx_next, phy_next, Cx, Cy = time_evolution(T, phx_previous, phy_previous)
