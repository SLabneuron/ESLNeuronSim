...
def calc_time_evolution_eca(init_X, init_Y, init_P, init_Q, init_phX, init_phY,
    ...
    def dde_X():
        ...
        Fin = np.where(Fin == 0, 0, gamma_X/Fin*(Tc/Tx))
    ...
    def dde_Y():
        ...
        Gin = np.where(Gin == 0, 0, gamma_Y/Gin*(Tc/Ty))

        # Second: if Gin > 0
        positive_condition = np.floor(Gin) >= M - 1
        Gin = np.where(positive_condition, 0, Gin)

        # Third: if Gin < 0
        negative_condition = np.ceil(Gin) <= -(M - 1)
        Gin = np.where(negative_condition, 0, Gin)
