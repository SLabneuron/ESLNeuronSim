...
@njit
def calc_time_evolution_ode(init_x, init_y, h,
    ...
    for i in range(total_step):
        ...
        # calculate
        x_next = x_previous + h * (1 / tau1) * ((b1 * S + WE11 * x_previous**2 + WE12 * y_previous**2) * (1 - x_previous) - (1 + WI11 * x_previous**2 + WI12 * y_previous**2) * x_previous)
        y_next = y_previous + h * (1 / tau2) * ((b2 * S + WE21 * x_previous**2 + WE22 * y_previous**2) * (1 - y_previous) - (1 + WI21 * x_previous**2 + WI22 * y_previous**2) * y_previous)
